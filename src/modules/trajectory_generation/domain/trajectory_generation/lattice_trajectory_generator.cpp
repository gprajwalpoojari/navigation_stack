...
#include <cmath>
#include <common/math_utils/geometry.hpp>

namespace trajectory_generation::trajectory_generation{
    TrajectoryGenerator::TrajectoryGenerator(const core_datastructures::Posture& start, 
                                                const core_datastructures::Posture& goal) {
        
        start_velocity = 0;
        start_time = 0
        spline_generator = std::make_shared<trajectory_generation::spline_generation::CubicSplineGenerator>(start, goal);
    }
    ...
    std::vector<core_datastructures::DynamicPosture> apply_acceleration(std::vector<core_datastructures::Posture>& spline, double acceleration) {
        ...
        for (int i = 1; i < spline.size(); i++) {
            ...
            double next_velocity = std::sqrt(std::pow(trajectory[i-1], 2) + 2 * acceleration * arc_length);
    ...
    std::vector<core_datastructures::DynamicPosture> TrajectoryGenerator::generate_trajectory(
                                                            const core_datastructures::DynamicPosture& start_dyn_posture, 
                                                            const core_datastructures::Posture& goal, double acceleration) {
        current_velocity = start_dyn_posture.v;
        current_time = start_dyn_posture.t;
        core_datastructures::Posture start{start_dyn_posture.x, start_dyn_posture.y, 
                                            start_dyn_posture.theta, start_dyn_posture.kappa};
